...
abstract class BaseImagen extends BaseObject  implements Persistent {
  ...
  const PEER = 'ImagenPeer';
	...
	public function setOpinionId($v)
	{
		...
		if ($this->aOpinion !== null && $this->aOpinion->getId() !== $v) {
			$this->aOpinion = null;
		}
	...
	public function ensureConsistency()
	{
		...
		if ($this->aOpinion !== null && $this->opinion_id !== $this->aOpinion->getId()) {
			$this->aOpinion = null;
		}
	} // ensureConsistency
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aOpinion !== null) {
				if ($this->aOpinion->isModified() || $this->aOpinion->isNew()) {
					$affectedRows += $this->aOpinion->save($con);
				}
				$this->setOpinion($this->aOpinion);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aOpinion !== null) {
				if (!$this->aOpinion->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aOpinion->getValidationFailures());
				}
			}
	 ...
	 * @throws     PropelException
	 */
	public function setOpinion(Opinion $v = null)
	{
		if ($v === null) {
	 ...
	 * @throws     PropelException
	 */
	public function getOpinion(PropelPDO $con = null)
	{
		if ($this->aOpinion === null && ($this->opinion_id !== null)) {
			$c = new Criteria(OpinionPeer::DATABASE_NAME);
			$c->add(OpinionPeer::ID, $this->opinion_id);
			$this->aOpinion = OpinionPeer::doSelectOne($c, $con);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aOpinion->addImagens($this);
			 */
		}
