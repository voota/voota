...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array EtiquetaSfGuardUser[] Collection to store aggregation of EtiquetaSfGuardUser objects.
	 */
	protected $collEtiquetaSfGuardUsers;

	/**
	 * @var        Criteria The criteria used to select the current contents of collEtiquetaSfGuardUsers.
	 */
	private $lastEtiquetaSfGuardUserCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collEtiquetaSfGuardUsers = null;
			$this->lastEtiquetaSfGuardUserCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collEtiquetaSfGuardUsers !== null) {
				foreach ($this->collEtiquetaSfGuardUsers as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getEtiquetaSfGuardUsers() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addEtiquetaSfGuardUser($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collEtiquetaSfGuardUsers collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addEtiquetaSfGuardUsers()
	 */
	public function clearEtiquetaSfGuardUsers()
	{
		$this->collEtiquetaSfGuardUsers = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collEtiquetaSfGuardUsers collection (array).
	 *
	 * By default this just sets the collEtiquetaSfGuardUsers collection to an empty array (like clearcollEtiquetaSfGuardUsers());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initEtiquetaSfGuardUsers()
	{
		$this->collEtiquetaSfGuardUsers = array();
	}

	/**
	 * Gets an array of EtiquetaSfGuardUser objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related EtiquetaSfGuardUsers from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array EtiquetaSfGuardUser[]
	 * @throws     PropelException
	 */
	public function getEtiquetaSfGuardUsers($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collEtiquetaSfGuardUsers === null) {
			if ($this->isNew()) {
			   $this->collEtiquetaSfGuardUsers = array();
				...
				$criteria->add(EtiquetaSfGuardUserPeer::SF_GUARD_USER_ID, $this->id);

				EtiquetaSfGuardUserPeer::addSelectColumns($criteria);
				$this->collEtiquetaSfGuardUsers = EtiquetaSfGuardUserPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				EtiquetaSfGuardUserPeer::addSelectColumns($criteria);
				if (!isset($this->lastEtiquetaSfGuardUserCriteria) || !$this->lastEtiquetaSfGuardUserCriteria->equals($criteria)) {
					$this->collEtiquetaSfGuardUsers = EtiquetaSfGuardUserPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastEtiquetaSfGuardUserCriteria = $criteria;
		return $this->collEtiquetaSfGuardUsers;
	}

	/**
	 * Returns the number of related EtiquetaSfGuardUser objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related EtiquetaSfGuardUser objects.
	 * @throws     PropelException
	 */
	public function countEtiquetaSfGuardUsers(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collEtiquetaSfGuardUsers === null) {
			if ($this->isNew()) {
				...
				$count = EtiquetaSfGuardUserPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastEtiquetaSfGuardUserCriteria) || !$this->lastEtiquetaSfGuardUserCriteria->equals($criteria)) {
					...
					$count = count($this->collEtiquetaSfGuardUsers);
				}
			} else {
				$count = count($this->collEtiquetaSfGuardUsers);
			}
	...
	/**
	 * Method called to associate a EtiquetaSfGuardUser object to this object
	 * through the EtiquetaSfGuardUser foreign key attribute.
	 *
	 * @param      EtiquetaSfGuardUser $l EtiquetaSfGuardUser
	 * @return     void
	 * @throws     PropelException
	 */
	public function addEtiquetaSfGuardUser(EtiquetaSfGuardUser $l)
	{
		if ($this->collEtiquetaSfGuardUsers === null) {
			$this->initEtiquetaSfGuardUsers();
		}
		if (!in_array($l, $this->collEtiquetaSfGuardUsers, true)) { // only add it if the **same** object is not already associated
			array_push($this->collEtiquetaSfGuardUsers, $l);
			$l->setsfGuardUser($this);
		}
	}
	 ...
	 * Otherwise if this sfGuardUser is new, it will return
	 * an empty collection; or if this sfGuardUser has previously
	 * been saved, it will retrieve related EtiquetaSfGuardUsers from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in sfGuardUser.
	 */
	public function getEtiquetaSfGuardUsersJoinEtiqueta($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collEtiquetaSfGuardUsers === null) {
			if ($this->isNew()) {
				...
				$this->collEtiquetaSfGuardUsers = EtiquetaSfGuardUserPeer::doSelectJoinEtiqueta($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collEtiquetaSfGuardUsers) {
				foreach ((array) $this->collEtiquetaSfGuardUsers as $o) {
					$o->clearAllReferences($deep);
				}
			}
