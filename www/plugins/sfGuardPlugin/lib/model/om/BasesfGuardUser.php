...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	 ...
	 */
	protected $collPropuestas;

	/**
	 * @var        Criteria The criteria used to select the current contents of collPropuestas.
	 */
	private $lastPropuestaCriteria = null;
	...
	/**
	 * @var        array EtiquetaPropuesta[] Collection to store aggregation of EtiquetaPropuesta objects.
	 */
	protected $collEtiquetaPropuestas;

	/**
	 * @var        Criteria The criteria used to select the current contents of collEtiquetaPropuestas.
	 */
	private $lastEtiquetaPropuestaCriteria = null;

	/**
	 * @var        array SfApplication[] Collection to store aggregation of SfApplication objects.
	 */
	protected $collSfApplications;

	/**
	 * @var        Criteria The criteria used to select the current contents of collSfApplications.
	 */
	private $lastSfApplicationCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collPropuestas !== null) {
				foreach ($this->collPropuestas as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collSfApplications !== null) {
				foreach ($this->collSfApplications as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getPropuestas() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addPropuesta($relObj->copy($deepCopy));
				}
			}
			...
			foreach ($this->getEtiquetaPropuestas() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addEtiquetaPropuesta($relObj->copy($deepCopy));
				}
			}

			foreach ($this->getSfApplications() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addSfApplication($relObj->copy($deepCopy));
				}
			}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Enlaces from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	...
	/**
	 * Clears out the collPropuestas collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addPropuestas()
	 */
	public function clearPropuestas()
	{
		$this->collPropuestas = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collPropuestas collection (array).
	 *
	 * By default this just sets the collPropuestas collection to an empty array (like clearcollPropuestas());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initPropuestas()
	{
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Propuestas from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getPropuestas($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collPropuestas === null) {
			if ($this->isNew()) {
			   $this->collPropuestas = array();
				...
				$this->collPropuestas = PropuestaPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	 ...
	 * @throws     PropelException
	 */
	public function countPropuestas(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collPropuestas === null) {
			...
			if (!$this->isNew()) {
				...
				if (!isset($this->lastPropuestaCriteria) || !$this->lastPropuestaCriteria->equals($criteria)) {
					...
					$count = count($this->collPropuestas);
				}
			} else {
				$count = count($this->collPropuestas);
			}
	 ...
	 * @throws     PropelException
	 */
	public function addPropuesta(Propuesta $l)
	{
		if ($this->collPropuestas === null) {
			$this->initPropuestas();
		}
		if (!in_array($l, $this->collPropuestas, true)) { // only add it if the **same** object is not already associated
			array_push($this->collPropuestas, $l);
			$l->setsfGuardUser($this);
		}
	}
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getPropuestasJoinPartidoRelatedByPartidoVideo1Id($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collPropuestas === null) {
			if ($this->isNew()) {
				...
				$this->collPropuestas = PropuestaPeer::doSelectJoinPartidoRelatedByPartidoVideo1Id($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Politicos from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collPoliticos === null) {
			if ($this->isNew()) {
			   $this->collPoliticos = array();
				...
				$criteria->add(PoliticoPeer::SF_GUARD_USER_ID, $this->id);
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getPoliticosJoinPartido($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collPoliticos === null) {
			if ($this->isNew()) {
				...
				$this->collPoliticos = PoliticoPeer::doSelectJoinPartido($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related EtiquetaPoliticos from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collEtiquetaPoliticos === null) {
			if ($this->isNew()) {
			   $this->collEtiquetaPoliticos = array();
				...
				$criteria->add(EtiquetaPoliticoPeer::SF_GUARD_USER_ID, $this->id);
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getEtiquetaPoliticosJoinPolitico($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collEtiquetaPoliticos === null) {
			if ($this->isNew()) {
				...
				$this->collEtiquetaPoliticos = EtiquetaPoliticoPeer::doSelectJoinPolitico($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related EtiquetaPartidos from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collEtiquetaPartidos === null) {
			if ($this->isNew()) {
			   $this->collEtiquetaPartidos = array();
				...
				$criteria->add(EtiquetaPartidoPeer::SF_GUARD_USER_ID, $this->id);
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getEtiquetaPartidosJoinPartido($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collEtiquetaPartidos === null) {
			if ($this->isNew()) {
				...
				$this->collEtiquetaPartidos = EtiquetaPartidoPeer::doSelectJoinPartido($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collEtiquetaPropuestas collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addEtiquetaPropuestas()
	 */
	public function clearEtiquetaPropuestas()
	{
		$this->collEtiquetaPropuestas = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collEtiquetaPropuestas collection (array).
	 *
	 * By default this just sets the collEtiquetaPropuestas collection to an empty array (like clearcollEtiquetaPropuestas());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initEtiquetaPropuestas()
	{
		$this->collEtiquetaPropuestas = array();
	}

	/**
	 * Gets an array of EtiquetaPropuesta objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related EtiquetaPropuestas from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array EtiquetaPropuesta[]
	 * @throws     PropelException
	 */
	public function getEtiquetaPropuestas($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collEtiquetaPropuestas === null) {
			if ($this->isNew()) {
			   $this->collEtiquetaPropuestas = array();
				...
				$this->collEtiquetaPropuestas = EtiquetaPropuestaPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Returns the number of related EtiquetaPropuesta objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related EtiquetaPropuesta objects.
	 * @throws     PropelException
	 */
	public function countEtiquetaPropuestas(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collEtiquetaPropuestas === null) {
			if ($this->isNew()) {
				...
				$count = EtiquetaPropuestaPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastEtiquetaPropuestaCriteria) || !$this->lastEtiquetaPropuestaCriteria->equals($criteria)) {
					...
					$count = count($this->collEtiquetaPropuestas);
				}
			} else {
				$count = count($this->collEtiquetaPropuestas);
			}
	...
	/**
	 * Method called to associate a EtiquetaPropuesta object to this object
	 * through the EtiquetaPropuesta foreign key attribute.
	 *
	 * @param      EtiquetaPropuesta $l EtiquetaPropuesta
	 * @return     void
	 * @throws     PropelException
	 */
	public function addEtiquetaPropuesta(EtiquetaPropuesta $l)
	{
		if ($this->collEtiquetaPropuestas === null) {
			$this->initEtiquetaPropuestas();
		}
		if (!in_array($l, $this->collEtiquetaPropuestas, true)) { // only add it if the **same** object is not already associated
			array_push($this->collEtiquetaPropuestas, $l);
			$l->setsfGuardUser($this);
		}
	}
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getEtiquetaPropuestasJoinEtiqueta($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collEtiquetaPropuestas === null) {
			if ($this->isNew()) {
				...
				$this->collEtiquetaPropuestas = EtiquetaPropuestaPeer::doSelectJoinEtiqueta($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collSfApplications collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addSfApplications()
	 */
	public function clearSfApplications()
	{
		$this->collSfApplications = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collSfApplications collection (array).
	 *
	 * By default this just sets the collSfApplications collection to an empty array (like clearcollSfApplications());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initSfApplications()
	{
		$this->collSfApplications = array();
	}

	/**
	 * Gets an array of SfApplication objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related SfApplications from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array SfApplication[]
	 * @throws     PropelException
	 */
	public function getSfApplications($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collSfApplications === null) {
			if ($this->isNew()) {
			   $this->collSfApplications = array();
				...
				$criteria->add(SfApplicationPeer::SF_GUARD_USER_ID, $this->id);

				SfApplicationPeer::addSelectColumns($criteria);
				$this->collSfApplications = SfApplicationPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				SfApplicationPeer::addSelectColumns($criteria);
				if (!isset($this->lastSfApplicationCriteria) || !$this->lastSfApplicationCriteria->equals($criteria)) {
					$this->collSfApplications = SfApplicationPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastSfApplicationCriteria = $criteria;
		return $this->collSfApplications;
	}

	/**
	 * Returns the number of related SfApplication objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related SfApplication objects.
	 * @throws     PropelException
	 */
	public function countSfApplications(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collSfApplications === null) {
			if ($this->isNew()) {
				...
				$count = SfApplicationPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastSfApplicationCriteria) || !$this->lastSfApplicationCriteria->equals($criteria)) {
					...
					$count = count($this->collSfApplications);
				}
			} else {
				$count = count($this->collSfApplications);
			}
	...
	/**
	 * Method called to associate a SfApplication object to this object
	 * through the SfApplication foreign key attribute.
	 *
	 * @param      SfApplication $l SfApplication
	 * @return     void
	 * @throws     PropelException
	 */
	public function addSfApplication(SfApplication $l)
	{
		if ($this->collSfApplications === null) {
			$this->initSfApplications();
		}
		if (!in_array($l, $this->collSfApplications, true)) { // only add it if the **same** object is not already associated
			array_push($this->collSfApplications, $l);
			$l->setsfGuardUser($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related SfReviews from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getSfReviewsJoinSfReviewType($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collSfReviews === null) {
			if ($this->isNew()) {
				...
				$this->collSfReviews = SfReviewPeer::doSelectJoinSfReviewType($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collPropuestas) {
				foreach ((array) $this->collPropuestas as $o) {
					$o->clearAllReferences($deep);
				}
			}
			...
			if ($this->collEtiquetaPropuestas) {
				foreach ((array) $this->collEtiquetaPropuestas as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collSfApplications) {
				foreach ((array) $this->collSfApplications as $o) {
					$o->clearAllReferences($deep);
				}
			}
