...
<li>
	<div class="avatar">
    	<?php echo image_tag('http://'.S3Voota::getBucketPub().'.s3.amazonaws.com/'.$reviewable->getPath().'/cc_s_'.($reviewable->getImagen()!=''?$reviewable->getImagen():'p_unknown.png'), 'alt="'. __('Foto de %1%', array('%1%' => $reviewable)) .'"') ?>
    </div>
	<h4 class="name"><?php echo link_to(cutToLength($reviewable->getLongName()), $reviewable->getModule().'/show?id='.$reviewable->getVanity())?></h4>
    <p class="votes">
  		<?php include_partial('home/sparkline_box', array('id' => $id)) ?>
