...
class perfilActions extends SfVoActions
{
  ...
  public function executeShow(sfWebRequest $request)
  {
    ...
    $this->response->setTitle( sfContext::getInstance()->getI18N()->__('Página de usuario de %1% en Voota', array('%1%' => trim($this->user)?$this->user:$user->getProfile()->getVanity())) );
    $descripcion = SfVoUtil::cutToLength($userProfile->getPresentacion(), 155, '...', true);
    $this->response->addMeta('Description', $descripcion?$descripcion:sfContext::getInstance()->getI18N()->__('Votos a favor y encontra de %1% sobre políticos y partidos de España', array('%1%' => trim($this->user)?$this->user:$user->getProfile()->getVanity())) );
  }
